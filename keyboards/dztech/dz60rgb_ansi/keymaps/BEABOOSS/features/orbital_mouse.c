...
enum {
    ...
    SLOW_MOVE_FACTOR_Q_8 = (ORBITAL_MOUSE_SLOW_MOVE_FACTOR) < 0.99 ? ((uint8_t)((ORBITAL_MOUSE_SLOW_MOVE_FACTOR) * 256 + 0.5)) : 255,
    /** Slow mode turn speed factor as a Q.8 value. */
    SLOW_TURN_FACTOR_Q_8 = (ORBITAL_MOUSE_SLOW_TURN_FACTOR) < 0.99 ? ((uint8_t)((ORBITAL_MOUSE_SLOW_TURN_FACTOR) * 256 + 0.5)) : 255,
...
bool process_orbital_mouse(uint16_t keycode, keyrecord_t* record) {
    if (!(IS_MOUSE_KEYCODE(keycode) || (ORBITAL_MOUSE_KEYCODE_RANGE_START <= keycode && keycode <= ORBITAL_MOUSE_KEYCODE_RANGE_END))) {
        return true;
    }
